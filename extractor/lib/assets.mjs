  ...
  for (const [key, value] of Object.entries(asset)) {
    if (key === "unitEnumName") {
      const types = await readCSTypes(
        `./SongsOfConquest/ExportedProject/Assets/Scripts/AutoGeneratedDef/SongsOfConquest/${asset.unitEnumName}.cs`
      );
      unitTypes[asset.languageKey] = types;
    }
    if (key === "visualsPath") {
      ...
      if (!asset.visuals.prefab) {
        ...
        const unitType = unitTypes[faction][asset.id];
        if (!unitType) {
          console.warn(
            `Could not find unit type for ${value} ${asset.languageKey}`
          );
          continue;
        }
        ...
        try {
          ...
          //
          prefabName = `${faction}${unitType.replace(faction, "")}`;
        }
        ...
        try {
          ...
          console.warn(
            `Could not find prefab for ${value} ${prefabName} ${asset.languageKey}`
          );
          if (!asset.visuals.prefab) {
...
const resolveTypes = async (asset) => {
  ...
  if (asset.commanderEnumName) {
    ...
    for (let commander of asset.commanders) {
      let type = types[commander.id];
      if (type.startsWith("Wielder")) {
        // Vanir doesn't have the right type in the commander enum
        type = commander.mName.replace("Definition", "");
      }
      commander.type = type;
    }
  }
